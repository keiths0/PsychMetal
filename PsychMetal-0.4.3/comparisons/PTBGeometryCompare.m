...
fprintf('Current: %.0f points wide, %.0f pixels wide; largest available %.0f\n', ...
    m.modePointWidth, m.modePixelWidth, m.largestModePixelWidth);
...
if isfinite(m.modePixelWidth) && isfinite(m.largestModePixelWidth) && ...
        abs(m.modePixelWidth - m.largestModePixelWidth) > 1
 fprintf(['SCALED mode: macOS renders a %.0f-pixel-wide framebuffer and resamples\n' ...
     ...
     m.modePixelWidth, m.largestModePixelWidth);
else
...
try
 ...
 for k = 1:max(1, round(holdSeconds / ifi))
  PsychMetal('FillRect', w, 255 * mod(k,2));
  PsychMetal('Flip', w);
 end
