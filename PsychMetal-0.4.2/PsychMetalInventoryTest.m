...
try
 ...
 reject('a colour in the filterMode slot is rejected', ...
     @() PsychMetal('DrawTexture', w, tex, [], box, 0, [255 0 0 255]));
 ...
 check('KbQueueRelease before creation', @() PsychMetal('KbQueueRelease'));
 reject('KbQueueStart before creation rejected', @() PsychMetal('KbQueueStart'));
 reject('KbQueueCreate with short mask rejected', @() PsychMetal('KbQueueCreate', zeros(1,255)));
 reject('KbQueueCreate with nonfinite mask rejected', @() PsychMetal('KbQueueCreate', nan(1,256)));
 reject('KbQueueCreate with too short interval rejected', @() PsychMetal('KbQueueCreate', zeros(1,256), 0.0001));
 reject('KbQueueCreate with too long interval rejected', @() PsychMetal('KbQueueCreate', zeros(1,256), 0.2));
 reject('KbQueueCreate with spare argument rejected', @() PsychMetal('KbQueueCreate', zeros(1,256), 0.002, 99));
 check('KbQueueCreate default arguments', @() PsychMetal('KbQueueCreate'));
 check('KbQueueCreate replaces queue with zero mask', @() PsychMetal('KbQueueCreate', false(256,1), 0.002));
 check('KbQueueCheck initially empty', @() assertQueueSummaryEmpty());
 check('KbQueueGetEvents initially empty', @() assertQueueEventsEmpty());
 check('KbQueueStart', @() PsychMetal('KbQueueStart'));
 check('KbQueueStart while running', @() PsychMetal('KbQueueStart'));
 PsychMetal('WaitSecs', 0.02);
 check('KbQueueCheck with zero mask', @() assertQueueSummaryEmpty());
 check('KbQueueGetEvents with zero mask', @() assertQueueEventsEmpty());
 check('KbQueueFlush while running', @() PsychMetal('KbQueueFlush'));
 check('KbQueueStop', @() PsychMetal('KbQueueStop'));
 check('KbQueueStop while stopped', @() PsychMetal('KbQueueStop'));
 check('KbQueueGetEvents after stop', @() assertQueueEventsEmpty());
 check('KbQueueCheck after stop', @() assertQueueSummaryEmpty());
 check('KbQueueFlush while stopped', @() PsychMetal('KbQueueFlush'));
 ...
 for qi = 1:numel(queueCommands)
  ...
  reject([queueCommand ' with spare argument rejected'], @() PsychMetal(queueCommand, 99));
 end
 check('KbQueueRelease', @() PsychMetal('KbQueueRelease'));
 check('KbQueueRelease repeated', @() PsychMetal('KbQueueRelease'));
 for qi = 1:numel(queueCommands)
  queueCommand = queueCommands{qi};
  if strcmp(queueCommand, 'KbQueueRelease'), continue; end
  reject([queueCommand ' after release rejected'], @() PsychMetal(queueCommand));
 end
 ...
 covered = lower({'Version','OpenWindow','BackgroundColor', ...
     ...
     'KbQueueCreate','KbQueueStart','KbQueueStop','KbQueueFlush', ...
     'KbQueueRelease','KbQueueGetEvents','KbQueueCheck', ...
 ...
 try, PsychMetal('KbQueueRelease'); catch, end
