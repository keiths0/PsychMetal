# PsychMetal 0.4.2 — a direct Metal replacement for Screen, on Apple silicon

## 0.4.2 mouse fix and keyboard queue

Mouse polling uses CoreGraphics directly and maps to Metal render dimensions.
The main MEX also includes a background keyboard queue that retains press and
release transitions while Octave/MATLAB renders or waits. `KbCheck` keeps its
instantaneous-state behavior. No standalone keyboard helper is required.

See [KEYBOARD-QUEUE.md](KEYBOARD-QUEUE.md) for the API, lifecycle, polling
limitations and overflow handling. Run `PsychMetalMouseRectDemo(20)` for mouse
input and `PsychMetalKbQueueDemo(15)` for quick T taps during host waits.

Both Apple Silicon binaries are rebuilt from this source. After replacing the
package, close any open PsychMetal window, then clear PsychMetal/PsychMetalCore
and ensure both `which` results resolve to this folder. The version remains
0.4.2; `PsychMetal('KbQueueCreate?')` confirms this updated build.

The k-space programs and example images are separate user applications and
are not included in the library release. Release tooling lives beside this
folder, never inside the library package.
