...
try
 ...
 for k = 1:total
  ...
  for j = 1:3
   ...
   % modulateColor is argument 8, after filterMode and globalAlpha, exactly
   % as in Screen('DrawTexture'), and it is on the window's ColorRange.
   PsychMetal('DrawTexture', w, texRGB, [], ...
       [mxc-s2, myc-s2, mxc+s2, myc+s2], -t * 30, 1, [], ...
       255 * [1, 1 - 0.3*j, 0.3*j, 1]);
  end
  ...
  if c >= 0
   tint = 255 * [1 1 1 min(1, 2*c)];
  else
   tint = 255 * [0 0 0 min(1, -2*c)];
  end
  PsychMetal('DrawTexture', w, texBlob, [], ...
      [bx0-side, by0-side, bx0+side, by0+side], 0, 1, [], tint);
