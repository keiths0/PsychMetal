...
% Version 0.4.2. SPDX-License-Identifier: MIT.
%
% PsychMetal
...
switch lower(command)
 ...
 case 'drawtexture'
  % PsychMetal('DrawTexture', w, texture [, srcRect] [, dstRect] [, angle] ...
  ...
  if numel(varargin) >= 6 && ~isempty(varargin{6})
   ...
   % A colour here is code written against the pre-0.4.0 signature, where the
   % tint sat at 6. Saying so is worth more than the generic complaint, which
   % sends the reader off to think about filter modes.
 ...
 case 'getmouse'
  ...
  % without conversion.
  %
  % CoreGraphics supplies current pointer/button state without AppKit event
  % dispatch. Positions are converted to the same render-pixel dimensions
  % used by OpenWindow, including scaled Retina and secondary displays.
  % buttons is logical [left right middle]. This is instantaneous polling,
  % not a click queue: a press and release between polls can be missed.
  assert(~isempty(S), 'PsychMetal is not open.');
  assert(~isempty(varargin) && isscalar(varargin{1}) && varargin{1} == S.buffer, ...
 ...
 case 'kbqueuecreate'
  ...
  assert((isnumeric(mask)||islogical(mask)) && isreal(mask) && numel(mask)==256 && all(isfinite(mask(:))), 'keyMask must have 256 finite real entries.');
...
fprintf('      [, angle] [, filterMode] [, globalAlpha] [, modulateColor]);\n');
...
fprintf('  KbQueueCreate / Start / Stop / Check / GetEvents / Flush / Release\n');
...
switch lower(strtrim(name))
 ...
 case {'getmouse','hidecursor','showcursor'}
  ...
  fprintf('  PsychMetal(''HideCursor'');\n');
  fprintf('  PsychMetal(''ShowCursor'');\n\n');
  fprintf(['All three read or set the pointer through Core Graphics\n' ...
      ...
      'Buttons are a logical [left right middle] vector, polled directly from\n' ...
      'the current session without AppKit event dispatch. Coordinates use the\n' ...
      'OpenWindow render dimensions. This is instantaneous polling, not a click\n' ...
      'queue: a press and release entirely between reads can be missed.\n\n' ...
...
fprintf('  [x, y, buttons] = PsychMetal(''GetMouse'', w);\n\n');
 ...
 case {'kbqueuecreate','kbqueuestart','kbqueuestop','kbqueueflush','kbqueuerelease','kbqueuegetevents','kbqueuecheck'}
  fprintf(['Keyboard event queue (all keyboards merged, HID indices as KbCheck).\n' ...
    'PsychMetal(''KbQueueCreate'' [, keyMask256] [, pollInterval=.002]);\n' ...
    'PsychMetal(''KbQueueStart''); PsychMetal(''KbQueueStop'');\n' ...
    '[events,dropped] = PsychMetal(''KbQueueGetEvents'');\n' ...
    'events is Nx3: [detectionTime HIDkeyCode pressed]. Retrieval drains FIFO.\n' ...
    '[pressed,firstPress,firstRelease,lastPress,lastRelease] = PsychMetal(''KbQueueCheck'');\n' ...
    'Check clears its 1x256 timestamp summaries, not the event FIFO.\n' ...
    'PsychMetal(''KbQueueFlush''); PsychMetal(''KbQueueRelease'');\n' ...
    'Start clears data and establishes current held-key baseline; holds do not repeat.\n' ...
    'Stop retains events. Flush clears events, summaries and dropped count.\n' ...
    'FIFO keeps newest 4096 events; dropped counts losses since Create/Start/Flush.\n' ...
    'Worker polls independently of rendering; timestamps are detection times in\n' ...
    'GetSecs units, NOT hardware event times. Pulses shorter than polling can be missed.\n' ...
    'No per-device selection; this is not full Psychtoolbox KbQueue compatibility.\n' ...
    'Close also releases the queue. See KEYBOARD-QUEUE.md and PsychMetalKbQueueDemo.\n']);
