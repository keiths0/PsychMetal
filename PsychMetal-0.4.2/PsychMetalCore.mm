// PsychMetalCore 0.4.2 — direct Metal presentation. No OpenGL.
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    @autoreleasepool {
        ...
        if (cmd(prhs[0], "Mouse")) {
            // Mouse() -> [x, y, buttons], render pixels, origin at top left.
            // Poll WindowServer directly. NSEvent's pressedMouseButtons is
            // cached AppKit state and can stay stale in an Octave/MATLAB loop
            // that does not dispatch AppKit events. Do not pump arbitrary UI
            // events here just to make a hardware-state read work.
            if (nrhs != 1 || nlhs != 3)
            ...
            // CGEventGetLocation and CGDisplayBounds share global desktop
            // coordinates (top-left origin, y down), including secondary
            // displays with negative origins. No AppKit y inversion needed.
            CGPoint p = CGEventGetLocation(event);
            CFRelease(event);
            // OpenWindow can choose a HiDPI mode's render dimensions rather
            // than CGDisplayPixelsWide/High. Use the SAME dimensions as every
            // drawing command. Leave off-display coordinates unclamped.
            ...
            // Retain [left, right, middle], logical 1x3, instantaneous state.
            // The combined session state includes trackpad/emulated clicks.
            // This is polling, not an event queue: short clicks between reads
            // can still be missed; adding latching here would change the API.
        ...
        if (cmd(prhs[0], "KbQueueCreate")) {
            if(nrhs!=3 || nlhs!=0 || !mxIsDouble(prhs[1]) || mxIsComplex(prhs[1]) || mxGetNumberOfElements(prhs[1])!=256)
